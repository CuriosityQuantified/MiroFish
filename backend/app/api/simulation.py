    ...
    Request (JSON):
        {
            ...
            "graph_id": "mirofish_xxxx",    // Optional, if not provided will be obtained from the project
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # The following statuses all indicate preparation work is completed:
        # - ready: Prepared, ready to run
        # - preparing: If config_generated=True, then completed
...
def prepare_simulation():
    ...
    Response:
        {
            ...
            "data": {
                ...
                "message": "Preparation task started|Completed preparation work already exists",
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Completed preparation work already exists, no need to regenerate",
        ...
        try:
            ...
            # Failure doesn't affect subsequent flow; the background task will re-fetch
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    If there are multiple, returns the latest one (sorted by created_at).
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    Differs from /profiles endpoint:
    - Reads files directly, not through SimulationManager
    - Suitable for real-time viewing during generation process
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation config (complete configuration intelligently generated by LLM).
...
def start_simulation():
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activities during simulation (posts, comments, likes, etc.) are updated to the Zep graph in real-time
        - This lets the graph "remember" the simulation process, useful for subsequent analysis or AI conversation
    ...
    try:
        ...
        # Smart status handling: if preparation work is completed, allow restart
        if state.status != SimulationStatus.READY:
            # Check if preparation work is completed
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)

            if is_prepared:
                # Preparation work completed; check if there's a running process
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is running; please call /stop endpoint first, or use force=true to force restart"
                            }), 400
...
def interview_agent():
    ...
    (completed simulation loop and entered command-waiting mode).
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or already closed. Please ensure simulation completed and entered command-waiting mode."
            }), 400
