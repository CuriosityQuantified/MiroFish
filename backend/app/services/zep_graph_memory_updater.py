...
class AgentActivity:
    ...
    def _describe_like_post(self) -> str:
        ...
        if post_content and post_author:
            return f"点赞了{post_author}'s帖子：「{post_content}」"
        elif post_content:
            return f"点赞了一条帖子：「{post_content}」"
        elif post_author:
            return f"点赞了{post_author}'s一条帖子"
        return "点赞了一条帖子"
    ...
    def _describe_dislike_post(self) -> str:
        ...
        if post_content and post_author:
            return f"踩了{post_author}'s帖子：「{post_content}」"
        elif post_content:
            return f"踩了一条帖子：「{post_content}」"
        elif post_author:
            return f"踩了{post_author}'s一条帖子"
        return "踩了一条帖子"
    ...
    def _describe_repost(self) -> str:
        ...
        if original_content and original_author:
            return f"转发了{original_author}'s帖子：「{original_content}」"
        elif original_content:
            return f"转发了一条帖子：「{original_content}」"
        elif original_author:
            return f"转发了{original_author}'s一条帖子"
        return "转发了一条帖子"
    
    def _describe_quote_post(self) -> str:
        """引用帖子 - 包含原帖内容、作者信息and引用评论"""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
        ...
        base = ""
        if original_content and original_author:
            base = f"引用了{original_author}'s帖子「{original_content}」"
        elif original_content:
            base = f"引用了一条帖子「{original_content}」"
        elif original_author:
            base = f"引用了{original_author}'s一条帖子"
        else:
            base = "引用了一条帖子"
    ...
    def _describe_create_comment(self) -> str:
        ...
        if content:
            if post_content and post_author:
                return f"在{post_author}'s帖子「{post_content}」下评论道：「{content}」"
            elif post_content:
                return f"在帖子「{post_content}」下评论道：「{content}」"
            elif post_author:
                return f"在{post_author}'s帖子下评论道：「{content}」"
    ...
    def _describe_like_comment(self) -> str:
        ...
        if comment_content and comment_author:
            return f"点赞了{comment_author}'s评论：「{comment_content}」"
        elif comment_content:
            return f"点赞了一条评论：「{comment_content}」"
        elif comment_author:
            return f"点赞了{comment_author}'s一条评论"
        return "点赞了一条评论"
    ...
    def _describe_dislike_comment(self) -> str:
        ...
        if comment_content and comment_author:
            return f"踩了{comment_author}'s评论：「{comment_content}」"
        elif comment_content:
            return f"踩了一条评论：「{comment_content}」"
        elif comment_author:
            return f"踩了{comment_author}'s一条评论"
        return "踩了一条评论"
...
class ZepGraphMemoryUpdater:
    ...
    监控模拟'sactions日志文件，将新'sagent活动实时更新到Zep图谱in。
    按平台分组，每累积BATCH_SIZE条活动后批量发送到Zep。
    
    所有有意义's行为都会被更新到Zep，action_argsin会包含完整's上下文信息：
    - 点赞/踩's帖子原文
    - 转发/引用's帖子原文
    - 关注/屏蔽's用户名
    - 点赞/踩's评论原文
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # 按平台分组's活动缓冲区（每个平台各自累积到BATCH_SIZE后批量发送）
        ...
        # 统计
        self._total_activities = 0  # 实际添加到队列's活动数
        self._total_sent = 0        # Success发送到Zep's批次数
        self._total_items_sent = 0  # Success发送到Zep's活动条数
        self._failed_count = 0      # SendFailed's批次数
        self._skipped_count = 0     # 被过滤跳过's活动数（DO_NOTHING）
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_argsin会包含完整's上下文信息（如帖子原文、用户名等）。
    ...
    def _flush_remaining(self):
        ...
        # 然后发送各平台缓冲区in剩余's活动（即使不足BATCH_SIZE条）
        with self._buffer_lock:
            for platform, buffer in self._platform_buffers.items():
                if buffer:
                    display_name = self._get_platform_display_name(platform)
                    logger.info(f"发送{display_name}平台剩余's {len(buffer)} 条活动")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_activities": self._total_activities,  # 添加到队列's活动总数
            "batches_sent": self._total_sent,            # Success发送's批次数
            "items_sent": self._total_items_sent,        # Success发送's活动条数
            "failed_count": self._failed_count,          # SendFailed's批次数
            "skipped_count": self._skipped_count,        # 被过滤跳过's活动数（DO_NOTHING）
    ...
    def stop_updater(cls, simulation_id: str):
        ...
        with cls._lock:
            if simulation_id in cls._updaters:
                ...
                logger.info(f"stopped图谱记忆更新器: simulation_id={simulation_id}")
    
    # 防止 stop_all 重复调用's标志
