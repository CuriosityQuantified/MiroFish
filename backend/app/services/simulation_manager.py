...
Using preset scripts + LLM smart config parameter generation
"""
...
class SimulationManager:
    ...
    3. Use LLM to smart-generate simulation config parameters
    ...
    def prepare_simulation(
        ...
        3. Use LLM to smart-generate simulation config parameters（时间、活跃度、发言频率等）
        ...
        Args:
            ...
            use_llm_for_profiles: YesNo使用LLM生成详细人设
            progress_callback: Progress callback (stage, progress, message)
            parallel_profile_count: Parallel persona generation count, default 3
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "No qualifying entities found, please check if graph was built correctly"
                self._save_simulation_state(state)
                return state
            ...
            # Reddit was saved in real-time during generation, save again here for completeness
            if progress_callback:
                progress_callback(
            ...
            # ========== Phase 3: LLM smart simulation config generation ==========
            if progress_callback:
                progress_callback(
                    "generating_config", 0, 
            ...
            # Note: run scripts remain in backend/scripts/ directory, no longer copied to sim directory
