...
API 1: Analyze text content, generate entity and relationship type definitions for social simulation
"""
...
ONTOLOGY_SYSTEM_PROMPT = """You are an expert knowledge graph ontology designer. Your task is to analyze the given text content and simulation requirements, then design entity types and relationship types suitable for a **social media public opinion simulation**.
...
- We simulate how various parties react to events and how information propagates

Therefore, **entities must be real-world actors that can speak and interact on social media**:

**Valid entity types**:
- Specific individuals (public figures, experts, journalists, ordinary people)
- Companies and businesses (including their official accounts)
- Organizations (universities, associations, NGOs, unions)
- Government agencies and regulators
- Media outlets (newspapers, TV channels, blogs, websites)
- Social media platforms themselves
- Specific community groups (alumni associations, fan groups, advocacy groups)
...
**Count: Must be exactly 10 entity types**
...
A. **Fallback types (required, place last in the list)**:
   - `Person`: Fallback for any individual person not covered by a more specific type
   - `Organization`: Fallback for any org not covered by a more specific type

B. **Specific types (8 types, designed based on text content)**:
   - Design based on the key roles that appear in the text
   - Example: for academic events: `Student`, `Professor`, `University`
   - Example: for business events: `Company`, `CEO`, `Employee`

**Why fallback types are needed**:
- Text may mention many unnamed individuals (e.g., "a teacher", "a bystander", "a netizen")
- Without a generic type, these can't be classified — they should fall into `Person`
- Similarly, small orgs and temporary groups should fall into `Organization`

**Specific type design principles**:
- Identify the high-frequency or key role types in the text
- Each specific type should have a clear boundary to avoid overlap
- The description must clearly explain how this type differs from the fallback
...
- Celebrity: Public figure or influencer
- Executive: Corporate leader or executive
- Official: Government official
- Lawyer: Legal professional
- Doctor: Medical professional

**Individual type (fallback)**:
- Person: Any individual not covered by a more specific type

**Organization types (specific)**:
- University: Higher education institution
- Company: Business or corporation
- GovernmentAgency: Government body or regulator
- MediaOutlet: News or media organization
- Hospital: Medical institution
- NGO: Non-governmental organization

**Organization type (fallback)**:
- Organization: Any org not covered by a more specific type
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            ...
            combined_text += f"\n\n...(original text was {original_length} chars; truncated to first {self.MAX_TEXT_LENGTH_FOR_LLM} chars for ontology analysis)..."
...
"""
        ...
        message += """
Based on the above content, design entity types and relationship types suitable for a social media opinion simulation.
...
"""
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        code_lines = [
            ...
            'Auto-generated by MiroFish for social simulation',
            '"""',
            '',
            'from pydantic import Field, BaseModel',
            '# Ontology models (compatible with both Graphiti and Zep)',
            'from pydantic import BaseModel as EntityModel',
            '',
            'EntityText = str  # Simple text attribute type',
            '',
            'class EdgeModel(BaseModel):',
            '    """Base edge model for ontology definitions"""',
            '    pass',
            '',
            '',
