...
- Does not shut down the environment immediately after simulation completes; enters command-waiting mode
...
if sys.platform == 'win32':
    ...
    # Reconfigure standard output streams to UTF-8 (fix console Chinese character garbling)
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/unspecified: Interview on both platforms simultaneously, return combined result
...
def _enrich_action_context(
    ...
    try:
        ...
        # Create comment: add the post information being commented on
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports both Anthropic and OpenAI backends, as well as dual-LLM configuration (boost mode).
    ...
    if provider == "anthropic" and not (use_boost and has_boost_config):
        ...
        if not os.environ.get("ANTHROPIC_API_KEY"):
            raise ValueError("Missing API Key configuration, please set ANTHROPIC_API_KEY or LLM_API_KEY")
        ...
        print(f"{config_label} [OpenAI] model={llm_model}, base_url={llm_base_url[:40] if llm_base_url else 'default'}...")
...
async def run_twitter_simulation(
    ...
    # If config doesn't have a certain agent, use OASIS default name
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Shut down environment immediately after simulation completes, do not enter command-waiting mode'
    )
