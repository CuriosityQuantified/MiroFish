    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? '启动in...' : '开始生成Result报告' }} 
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- 通用回退：未知Typeor有 content 但未被上述处理 -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // 从Step2传入's最大rounds数
  minutesPerRound: {
    type: Number,
...
const actionIds = ref(new Set()) // 用于去重's动作ID集合
const scrollContainer = ref(null)

// Computed
// 按Time顺序显示动作（最新's在最后面，即底部）
...
// 格式化模拟流逝Time（根据Roundand每roundsminutes数计算）
...
const doStartSimulation = async () => {
  ...
  // 先重置所有Status，确保不会受到上一次模拟's影响
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('正在启动双PlatformParallel Simulation...')
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ 已清理旧'sSimulation Log，重新Start Simulation')
      }
      addLog('✓ 模拟引擎started successfully')
      ...
      addLog(`✗ start failed: ${res.error || '未知错误'}`)
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`停止Failed: ${res.error || '未知错误'}`)
    }
  } catch (err) {
...
// 追踪各Platform's上一次Round，用于检测变化并输出日志
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 检测模拟YesNoCompleted（通过 runner_status orPlatform完成Status判断）
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // 额外检查：如果后端还没来得及更新 runner_status，但Platform已经Report Complete
      // 通过检测 twitter_completed and reddit_completed Status判断
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ 检测到所有Platform模拟已结束')
        }
        addLog('✓ 模拟Completed')
        phase.value = 2
        stopPolling()
...
// 检查所有启用'sPlatformYesNoCompleted
const checkPlatformsCompleted = (data) => {
  // 如果没有任何Platform数据，返回 false
  if (!data) return false
  
  // 检查各Platform's完成Status
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // 如果至少有一个Platform完成了，检查YesNo所有启用'sPlatform都完成了
  // 通过 actions_count 判断PlatformYesNo被启用（如果 count > 0 or running 曾为 true）
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // 如果没有任何Platform被启用，返回 false
  if (!twitterEnabled && !redditEnabled) return false
  
  // 检查所有启用'sPlatformYesNo都Completed
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Report Generation请求已Send，Please wait...')
    return
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✗ 启动Report GenerationFailed: ${res.error || '未知错误'}`)
...
onMounted(() => {
  addLog('Step3 Simulation Running初始化')
  if (props.simulationId) {
    doStartSimulation()
  }
})
