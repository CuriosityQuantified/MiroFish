    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            结合上下文，自动调用工具从Knowledge Graph梳理实体与关系，初始化模拟个体，并基于现实种子赋予他们独特's行为与记忆
          </p>
          ...
          <div v-if="profiles.length > 0" class="profiles-preview">
            <div class="preview-header">
              <span class="preview-title">Generated's Agent 人设</span>
            </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM 根据Simulation Requirement与现实种子，智能设置世界Time流速、推荐算法、每个个体's活跃Time段、发言频率、事件触发等参数
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        <div class="card-header">
          ...
          <div class="step-status">
            ...
            <span v-else-if="phase === 3" class="badge processing">编排in</span>
            <span v-else class="badge pending">等待</span>
          </div>
        </div>
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            基于叙事方向，自动生成初始激活事件与Hot Topics，引导模拟世界's初始Status
          </p>
          ...
          <div v-if="simulationConfig?.event_config" class="orchestration-content">
            ...
            <div class="topics-section">
              <span class="box-label">初始Hot Topics</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- 模拟rounds数配置 - 只有在Configuration Generation完成且rounds数计算出来后才显示 -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">模拟rounds数设定</span>
                <span class="section-desc">MiroFish 自动规划推演现实 <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours，每rounds代表现实 <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutesTime流逝</span>
              </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">若首次运行，强烈建议切换至‘自定义模式’减少模拟rounds数，以便快速Preview效果并降低报错风险 ➝</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              <div class="dimension-card">
                ...
                <span class="dim-desc">在此事件in's完整行为轨迹</span>
              </div>
              <div class="dimension-card">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">基于现实种子形成's记忆</span>
              </div>
              <div class="dimension-card">
...
watch(currentStage, (newStage) => {
  if (newStage === '生成Agent Persona' || newStage === 'generating_profiles') {
    phase.value = 1
  } else if (newStage === '生成Simulation Config' || newStage === 'generating_config') {
    phase.value = 2
    // 进入Configuration Generation阶段，开始rounds询配置
    if (!configTimer) {
      addLog('开始生成双PlatformSimulation Config...')
      startConfigPolling()
    }
...
// 从配置in计算自动生成'srounds数（不使用硬编码默认值）
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    // 用户选择保持自动生成'srounds数，不传递 max_rounds 参数
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog('开始rounds询准备Progress...')
      // 开始rounds询Progress
      startPolling()
      // 开始实时获取 Profiles
      startProfilesPolling()
    } else {
      addLog(`准备Failed: ${res.error || '未知错误'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        ...
        addLog(`✗ 准备Failed: ${data.error || '未知错误'}`)
        stopPolling()
        stopProfilesPolling()
      }
    }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        addLog(`→ Agent Persona ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || '未知职业'})`)
        
        // 如果全部生成完成
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(`✓ 全部 ${currentCount} 个AgentPersona Generation完成`)
        }
      }
    }
  } catch (err) {
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 输出Configuration Generation阶段日志（避免重复）
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        lastLoggedConfigStage = data.generation_stage
        if (data.generation_stage === 'generating_profiles') {
          addLog('正在生成Agent Persona配置...')
        } else if (data.generation_stage === 'generating_config') {
          addLog('正在调用LLM生成Simulation Config参数...')
        }
      }
      ...
      if (data.config_generated && data.config) {
        ...
        addLog('✓ 模拟Configuration Generation完成')
        ...
        if (data.summary) {
          ...
          addLog(`  └─ Platform配置: Twitter ${data.summary.has_twitter_config ? '✓' : '✗'}, Reddit ${data.summary.has_reddit_config ? '✓' : '✗'}`)
        }
        
        // 显示Time配置Details
        if (data.config.time_config) {
          const tc = data.config.time_config
          addLog(`Time配置: 每rounds${tc.minutes_per_round}minutes, Total${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)}rounds`)
        }
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // 配置尚未生成，开始rounds询
        addLog('Configuration Generationin，开始rounds询等待...')
        startConfigPolling()
      }
    }
  } catch (err) {
