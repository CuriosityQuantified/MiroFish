    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG长短期记忆实时更新in' : '实时更新in...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">还有少量ContentProcessing，建议稍后手动Refresh图谱</span>
          <button class="hint-close-btn" @click="dismissFinishedHint" title="CloseNote">
...
const expandedSelfLoops = ref(new Set()) // Expand's自环项
const showSimulationFinishedHint = ref(false) // 模拟结束后'sNote
const wasSimulating = ref(false) // 追踪之前YesNo在模拟in
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // 从模拟in变为非Simulation Status，显示结束Note
...
const renderGraph = () => {
  ...
  // 处理边数据，计算同一对节点间'sEdge Countand索引
  const edgePairCount = {}
  const selfLoopEdges = {} // 按节点分组's自环边
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // 统计每对节点之间'sEdge Count，收集自环边
  tempEdges.forEach(e => {
  ...
  // 记录当前处理到每对节点's第几条边
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // 已处理's自环节点
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // 存储所有自环边's详细信息
        }
      })
      return
    }
    ...
    // 判断边's方向YesNo与标准化方向一致（源UUID < 目标UUID）
    ...
    if (totalCount > 1) {
      // 均匀分布曲率，确保明显区分
      // 曲率范围根据Edge Count增加，边越多曲率范围越大
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // 如果边's方向与标准化方向相反，翻转曲率
  ...
  // Simulation - 根据Edge Count动态调整节点间距
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // 根据这对节点之间'sEdge Count动态调整距离
      // 基础距离 150，每多一条边增加 40
  ...
  const getLinkPath = (d) => {
    ...
    // 计算曲线控制点 - 根据Edge Countand距离动态调整
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // 垂直于连线方向's偏移，根据距离比例计算，保证曲线明显可见
    // 边越多，偏移量占距离's比例越大
