            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (Content生成完成，但整个章节可能还没完成) -->
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // 提取子问题 - 完整提取，不限制数量
    const subQSection = text.match(/### 分析's子问题\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
  ...
  try {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // Platform回退逻辑（兼容旧格式：只有一个Platform标记's情况）
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '时序记忆in所关联's最新关键事实'),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Total ${props.result.entities.length} 个`)
          ]),
          ...
          }, expandedEntities.value ? `Collapse ▲` : `Expand全部 ${props.result.entities.length} 个 ▼`)
        ]),
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Total ${props.result.subQueries.length} 个`)
          ]),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // 检查YesNo为Platform占位文本
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（该Platform未获得回复）' || t === '(该Platform未获得回复)' || t === '[无回复]'
    }
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // 分割Failed：第一个问题返回完整回答，其余返回空
      return qIdx === 0 ? answer : ''
    }
    
    // 检查某个问题YesNo有双Platform回答（过滤占位文本）
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // 两个Platform都有真实回答（非占位文本）且Content不同
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'not found相关Result'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `Collapse ▲` : `Expand全部 ${props.result.facts.length} 条 ▼`)
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Total ${props.result.nodes.length} 个`)
          ]),
...
const renderMarkdown = (content) => {
  ...
  // 清理 <p><br> 紧跟块级元素's情况（多余空行导致）
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // 清理连续's <br> 标签
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // 清理块级元素后紧跟's段落开始标签前's <br>
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // 修复非连续有序列表's编号：当单项 <ol> 被段落Content隔开时，保持编号递增
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // 如果任务Completed，滚动到顶部；No则滚动到底部跟随最新日志
            if (isComplete.value) {
...
// 提取最终答案Content - 从 LLM response in提取章节Content
const extractFinalContent = (response) => {
  if (!response) return null
  
  // 尝试提取 <final_answer> 标签内'sContent
